...
0. **`docs/plans/linked-variations.md` — Stages V.1 + V.2 + V.3 + V.4 + V.5 DONE + pushed 2026-09-13, V.2 and V.3 manually verified by the user (that verification found the V.5 bug, fixed same day).** Let two otherwise-independent recipes declare "this is a variation of that" (e.g. Bloody Mary / Bloody Mary (Practical Version)) as pure metadata + navigation - never ingredient/instruction inheritance, never affecting either recipe's own availability. **Read that file before starting the next stage** - it's the source of truth for this item.

   **V.1 (schema):** migration `20260913120000_recipe_relationships.sql` -
   ...
   `BEFORE INSERT OR UPDATE` cycle-prevention trigger (cheap because
   `unique(recipe_id)` makes the graph a forest of trees - one linked-list
   walk, not a general traversal). New pure
   `src/domain/recipeRelationships.js` (one-hop resolvers).

   **V.2 (editor assignment + atomicity fix):** `EditorScreen.jsx` gains a
   "Variation of" field + an optional note, draft-only until Save. A
   first-pass `set_recipe_variation_of()` RPC was found incomplete by the
   user before approval (a later recipe-field/component/tag failure could
   still leave the relationship half-updated) and replaced with one atomic
   `save_recipe(p_recipe_id, p_fields, p_components, p_taste_tag_ids,
   p_variation_of)` (migration `20260913140000`, mirrors
   `save_ingredient_type()`) owning the recipe's fields, components, taste
   tags, AND the relationship in ONE transaction; both
   `createRecipe()`/`updateRecipe()` now call this one RPC. **Manually
   verified by the user, 2026-09-13:** assignment saves correctly, the
   note round-trips.

   **V.3 (Cocktail Detail UI + navigation):** new
   `src/components/detail/VariationsSection.jsx` - "Variation of"
   (the one base, if any) and/or "Variations" (direct children, if any),
   both can show together for a recipe in the middle of a chain. Reuses
   the existing `CocktailCard` in the same grid Ingredient Detail's own
   "cocktails using this" section already established. Renders `null`
   when there's nothing to show. Placed in `DetailScreen.jsx` after
   `StepsSection`, before `ActionButtons`. **Zero new data loading** -
   `recipeRelationships` was already fetched since V.2; `DetailScreen.jsx`
   builds a local `recipesById` map from `computed` and calls V.1's
   `resolveRecipeVariationContext()`. **Manually verified by the user,
   2026-09-13:** variation → base and base → variation both display and
   navigate correctly; the reused `CocktailCard` (own live
   availability/makeability badge per card) confirmed to look good and be
   worth preserving as-is.

   **V.4 (makeability-aware presentation + first real catalogue links),
   2026-09-13:** presentation from V.3 preserved exactly, unredesigned.
   `resolveRecipeVariationContext()`'s `variations` sort is now
   makeability-tier-aware first (reusing the shared `DISPLAY_TIER_ORDER`,
   `src/domain/availabilityGroups.js` - perfect > good > adapted > almost
   > unavail, the same order every other "grouped by makeability" view
   already uses), alphabetical as the tie-break inside a tier - no new
   ranking system. New `shouldShowMakeableVariationFraming(recipe,
   variations)` (`src/domain/recipeRelationships.js`) reuses a newly
   extracted `isPossibleTier(tier)` helper
   (`src/domain/makeabilityCounts.js`, the same rule
   `summarizeMakeability()` already uses internally - perfect/good/adapted
   count as "possible," almost/unavail don't) to decide the one boolean
   the "Can't make the original? You can make one of these instead." line
   needs: shown only when the current recipe's own tier is NOT possible
   and at least one direct variation's tier IS - never when the base is
   already possible (strict or adapted), never when no variation is
   possible either. Rendered inside "Variations" only (base → variations
   side, per the plan - a variation's own page never editorializes about
   its base). `DetailScreen.jsx` computes it once, passes it down as a
   plain prop to `VariationsSection.jsx`.

   **Editor wording polish (small, isolated, included per explicit
   allowance):** the "Variation of" label/helper text was ambiguous about
   direction - now reads **"Based on / variation of (optional)"** with
   helper text making the direction explicit. **"How it differs
   (optional)"** left unchanged - manual testing confirmed it already
   works well. **Not built:** a sticky recipe-name header in the editor
   while scrolling - recorded as a separate small UX follow-up in the plan
   doc's own "Manual-test findings" section, out of scope for V.4.

   **Real catalogue relationships, both written via the app's own normal
   path (`save_recipe()` RPC, simulating a real authenticated admin
   identity the same way `rls_suite.sql` does, never a raw INSERT
   bypassing RLS):**
   - **Bloody Mary (Practical Version) → Bloody Mary** - already existed
     (created by the user during manual verification through the real
     editor). Reviewed, note is accurate, kept as-is (no duplicate
     created): "Uses Soy sauce instead of Worcestershire Sauce and regular
     salt instead of Celery Salt. More common at home."
   - **Zombie (Home Bar Spiced & Dark Spec) → Zombie** - newly linked
     after inspecting both recipes' actual ingredients/amounts/steps (no
     lore, no assumptions): the variation uses 2 rums (Dark + Spiced)
     instead of 3 (Dark + Gold + Demerara), Simple Syrup instead of
     Falernum + Cinnamon Syrup, omits Pernod, and is shaken/strained over
     ice instead of blended - genuine differences, both classic/shared/
     same family. Note: "Uses Dark Rum and Spiced Rum instead of Dark,
     Gold, and Demerara Rum, and Simple Syrup instead of Falernum,
     Cinnamon Syrup, and Pernod. Shaken and strained over ice instead of
     blended - easier with common home-bar ingredients and no blender
     needed." Written by rebuilding the variation's existing
     fields/components/taste tags directly from its own current rows (a
     like-for-like re-save, verified byte-for-byte unchanged afterward via
     direct SQL query) plus the new relationship - confirmed via SQL that
     no other content on either recipe changed.

   **V.5 (bugfix, 2026-09-13) - the user's own manual verification of V.4
   found a real presentation bug:** the stored relationship `note` is
   directional ("how the variation differs from its base"), but a
   variation's own "Variation of" block rendered that note as a caption
   directly under the BASE's card - visually implying the note described
   the base recipe, when it actually describes the CURRENT recipe (the
   variation). Concretely, opening Bloody Mary (Practical Version) showed
   "Uses Soy sauce instead of Worcestershire Sauce..." under the *Bloody
   Mary* card, when that fact is about the Practical Version, not Bloody
   Mary itself. Fixed in `src/components/detail/VariationsSection.jsx`
   (presentation only - no schema/domain/service change): the base's card
   now renders alone in its grid, with the note pulled into its own
   labeled **"How this version differs"** block below it. The
   base→variations direction (a base's page showing each variation's own
   note as a caption under that variation's card) was already correct and
   is unchanged. Never rewrites/inverts the note text - same stored string
   either way, only the layout differs by direction. 2 new domain tests
   (same note unchanged from both `resolveRecipeVariationContext()`
   ...
   **V.5 final wording-clarity polish (same day) - the user's own final
   manual testing found a third issue, this time in the wording itself:**
   even with the note correctly moved off the base card and constrained to
   column width, the generic heading **"How this version differs"** was
   still easy to misread as being about the BASE card shown directly above
   it, rather than the CURRENT recipe (the variation whose page it is).
   Fixed by naming the current recipe explicitly - **"How Bloody Mary
   (Practical Version) differs"**, **"How Zombie (Home Bar Spiced & Dark
   Spec) differs"** - via a new pure `formatVariationDifferenceHeading
   (currentRecipeName)` (`src/domain/recipeRelationships.js`);
   `DetailScreen.jsx` passes its own already-available `c.name` into
   `VariationsSection.jsx`'s new `currentRecipeName` prop. Deliberately
   does not truncate the name - unlike the sticky editor header (fixed-
   height chrome), this is ordinary page content, so an exceptionally long
   name is allowed to wrap onto a second line, still inside the same
   single grid-column width the layout-width polish already established.
   The base→variations side needed no change - reviewed, and that note is
   already unambiguously tied to the variation's own card it describes.
   3 new domain tests (names the current variation, never a base -
   ...
   **Not touched (out of scope, confirmed by inspection):** `computeAvail()`;
   ...
   recorded here was built separately as its own general Recipe Editor UX
...
## Last completed chunk (Adapted-card presentation follow-up - shorter wording + tighter icon/text layout, 2026-09-13 — buildAdaptedCardActions() text shortened, CocktailCard.jsx layout tweak, existing tests updated in place, no schema/eligibility/count change)

**Found by the user's own mobile manual testing of the previous adapted-
card fix.** The new concrete actions ("Use Spiced Rum", "Use Lemon Juice",
"Prepare Simple Syrup") were semantically correct but still too long for
the existing two-column mobile card width - they wrapped awkwardly (e.g.
"Use Spiced" / "Rum" on separate lines), even though ordinary Perfect/
Almost cards look fine in the same grid. The problem was specifically the
adaptation-action copy, not the card/grid itself.

**Wording shortened in the one shared helper - `buildAdaptedCardActions()`
(`src/domain/makeability.js`), still the single source every card surface
reads:** a substitute action's `text` is now just the bare replacement
name ("Spiced Rum", "Lemon Juice" - no "Use " prefix, since the card
already carries the shared "adapted" icon right next to it via `kind`,
and the section heading "Make With Adaptations" already establishes the
context); a preparation action is now `"Prep <name>"` (was "Prepare
<name>", no "first"). `kind` itself (`"substitute"` | `"preparation"`) is
completely unchanged - still what a caller reads to decide the icon.
`display.label`/`composeAdaptedLabel()` (the detail-page-facing composed
sentence, e.g. "Make with substitutions · Prepare Simple Syrup first")
are untouched - confirmed by the existing `computeMakeability()` tests for
`display.label` still passing unmodified, proving the detail page's own
richer wording is unaffected by this change.

**Layout tweak in `CocktailCard.jsx` only (not `SmallCard` - its action
text was never split across an icon/text boundary, since the icon there
is a plain string prefix inside one text node, not a separate flex
child):** the per-action row changed from `flex` to `inline-flex` (so the
icon can't detach from its own text onto a separate line) and from
"centered on mobile, left-aligned on desktop" (the card's usual
name/status convention) to unconditionally left-aligned - these are
compact status/action rows, not prose, so they don't need the same
centering treatment as the recipe name above them. Long ingredient names
still wrap normally if genuinely too long for the card - this only stops
the icon and an ordinarily-short name (now much shorter without "Use"/
"Prepare"/"first") from splitting apart unnecessarily. Card dimensions
and the two-column grid itself are completely untouched.

**Verified:** no new tests added - the existing 6
`buildAdaptedCardActions()` tests (added for the prior chunk) were
updated in place to assert the new compact strings, and continue to cover
substitution-only, preparation-only, substitution + preparation together
(recipe order), multiple substitutions (deterministic order), the
actually-owned substitute (not merely the first catalogue option), and
the empty-array case for no adaptation. `corepack pnpm@10.34.3 test`
**392/392** (same count - text updated in place, no test added or
removed). `corepack pnpm@10.34.3 build` clean (182 modules, unchanged).
Diff is exactly the same 3 files as the prior chunk
(`makeability.js`, `makeability.test.js`, `CocktailCard.jsx`) - confirmed
via `git status` before committing.

**Not touched (confirmed by inspection, same as the prior chunk):**
adaptation eligibility, `display.tier`, the resolved substitute choice
itself, preparation resolution, the missing-chip suppression logic (still
gated on `display.tier === "almost"`, untouched), Perfect/Good/Almost/
Unavailable presentation, `HeroCard`'s Cocktail Detail wording, Library/
Home grouping, card dimensions/grid, Linked Variations logic.

**Not yet browser/mobile-verified this stage:** open Library/Home's "Make
With Adaptations" cards and confirm a substitution-only card now shows
"⇄ Spiced Rum" (or "⇄ Lemon Juice") on one line, not split across two;
a preparation-only card shows "Prep Simple Syrup" on one line; a card
...
## Last completed chunk (Linked Variations Stage V.5 - final wording clarity, heading names the current variation recipe, 2026-09-13 — 3-file presentation change + 3 new domain tests, no schema/migration/catalogue change)

**Found by the user's own final manual testing - the third distinct
manual-verification finding in the V.5 arc.** The first fix moved the
relationship note off the base's card and into its own "How this version
differs" block (correct direction). The second fix constrained that block
to the card's own column width (correct sizing). Even with both fixes in
place, the user found the heading itself still ambiguous while testing:
"How this version differs" doesn't say WHICH recipe "this version" is -
it sits directly under the base's card, so it's still easy to misread as
being about the base, when it's actually always about the CURRENT recipe
(the variation whose page it is).

**Fix:** the heading now names the current recipe explicitly - **"How
Bloody Mary (Practical Version) differs"**, **"How Zombie (Home Bar
Spiced & Dark Spec) differs"** - removing the ambiguity outright, since
naming the variation makes it impossible to confuse with the base shown
above it.

**Implementation (3 files):**
- New pure `formatVariationDifferenceHeading(currentRecipeName)`
  (`src/domain/recipeRelationships.js`) - returns
  `` `How ${currentRecipeName} differs` ``, nothing else. Extracted as its
  own function (not an inline template string in the component) purely so
  this exact contract - the heading names the CURRENT recipe, never the
  base - is unit-testable without a component-rendering harness (this
  project has none). Structurally cannot name the base by mistake: the
  function takes exactly one recipe-name argument, confirmed by asserting
  its own arity in a test.
- `src/screens/DetailScreen.jsx` - passes its own already-available
  `c.name` (the CURRENT recipe's own name - no new lookup, no new fetch)
  into a new `currentRecipeName` prop on `VariationsSection.jsx`.
- `src/components/detail/VariationsSection.jsx` - the "Variation of"
  block's heading now calls `formatVariationDifferenceHeading
  (currentRecipeName)` instead of the static string "How this version
  differs".

**Long names / responsive behavior:** deliberately does NOT truncate the
name - unlike the sticky editor header (fixed-height chrome where
truncation was the right call), this is ordinary page content, so an
exceptionally long recipe name is allowed to wrap onto a second line
rather than hide which recipe the note describes. The heading still sits
inside the exact same single grid-column width the prior layout-width
polish already established, so it wraps safely without overflowing -
no new width/overflow handling needed, that constraint was already in
place.

**Base → variations direction reviewed, not changed:** on a base's own
page, each variation's own note is still a plain caption directly under
that variation's own card (no heading at all) - already unambiguous in
that direction (the note visibly belongs to the card right above it,
with no OTHER card competing for the reader's attention the way the base
card does on the variation side), so no change was needed or made there.

**Verified:** no new migration (no schema/RLS touched). `corepack
pnpm@10.34.3 test` **386/386** (+3: names the current variation, not a
base - confirmed structurally via the function's own one-argument arity;
does not truncate an exceptionally long name). `corepack pnpm@10.34.3
build` clean (182 modules, unchanged - no new file, `recipeRelationships
.js` already existed). Diff is exactly 3 files (`recipeRelationships.js`,
`DetailScreen.jsx`, `VariationsSection.jsx`) + the one test file -
confirmed via `git status` before committing. No change to the note text
itself, relationship direction, cards, makeability/ordering, schema, or
editor behavior.

**Not yet browser-verified:** open Bloody Mary (Practical Version) and
Zombie (Home Bar Spiced & Dark Spec), confirm the heading now reads "How
<this recipe's own name> differs" rather than the base's name, wraps
...
## Last completed chunk (Linked Variations Stage V.5 - bugfix: directional relationship note placement, 2026-09-13 — presentation-only change to 1 component + 2 new domain tests, no schema/migration/catalogue change)

**Found by the user's own manual verification of V.4.** The stored
`recipe_relationships.note` is directional - it always means "how the
variation differs from its base," regardless of which recipe's own
detail page is currently rendering it. V.3/V.4's "Variation of" block
rendered that note as a small caption directly under the BASE recipe's
`CocktailCard` - the same treatment a variation's own note correctly gets
under ITS OWN card on a base's "Variations" block. That treatment is
right in the base→variations direction (the note there really does
describe the card it sits under) but wrong in the variation→base
direction: the note describes the CURRENT recipe (the variation), not
the base card shown above it. Concretely: opening **Bloody Mary
(Practical Version)** showed "Uses Soy sauce instead of Worcestershire
Sauce and regular salt instead of Celery Salt. More common at home."
directly under the **Bloody Mary** card - visually implying Bloody Mary
itself uses soy sauce, when the opposite is true (Bloody Mary uses
Worcestershire and Celery Salt; the Practical Version is the one using
soy sauce and regular salt). Same problem for the Zombie pair.

**Fix (presentation only) - `src/components/detail/
VariationsSection.jsx`:** on a variation's own page, the base's
`CocktailCard` now renders alone in its grid (no per-card wrapper/caption
slot at all); `base.note`, if present, is pulled into its own explicitly
labeled block below the grid - **"How this version differs"** - so it can
no longer read as a caption on the base's card. The base→variations
direction is completely unchanged: each variation's own note is still a
small caption directly under that variation's own card, exactly as V.3
shipped it - that side was already correct and needed no new heading
(per the user's own explicit instruction not to repeat headings where the
existing layout already reads unambiguously). Deliberately does **not**
attempt to rewrite, invert, or regenerate the note text for either
direction - the stored note may not be mechanically invertible (e.g.
never auto-generating "Uses Worcestershire instead of Soy Sauce" from the
stored string), so both directions render the exact same stored string;
only the LAYOUT differs.

**Not touched, confirmed by inspection (per the explicit instruction this
is presentation-only):** the `recipe_relationships` schema/migrations;
the stored note text itself (both real rows, Bloody Mary and Zombie,
completely unchanged); the relationship's direction/FK; V.4's
makeability-tier ordering (`DISPLAY_TIER_ORDER` sort untouched);
V.4's "Can't make the original?" framing logic
(`shouldShowMakeableVariationFraming()` untouched); `CocktailCard` itself;
the live Bloody Mary/Zombie catalogue rows (no new SQL, no `save_recipe()`
call this stage at all).

**New tests** (`src/domain/recipeRelationships.test.js`): the same stored
note string resolves unchanged whether read via
`resolveRecipeVariationContext()` from the variation's own perspective
(`base.note`) or from the base's own perspective
(`variations[].note`) - proving the resolver itself never rewrites or
inverts the note regardless of direction; a note-less relationship
resolves to `note: null` cleanly on both sides, no crash, no placeholder
text. The actual layout change (note moved out from under the base's
card into its own labeled block) is a component-level, structural
change - matching this project's own honest, established testing limits
(no jsdom/component rendering available) - verified by reading the code,
not an automated render test.

**Verified:** no new migration (no schema/RLS touched at all this
stage - not re-run). `corepack pnpm@10.34.3 test` **377/377** (+2). `corepack
pnpm@10.34.3 build` clean (181 modules, unchanged - no new file). Diff
scope: exactly `src/components/detail/VariationsSection.jsx` (the fix) +
`src/domain/recipeRelationships.test.js` (the 2 new tests) - reviewed and
confirmed nothing else touched, matching the explicit "presentation fix
only" instruction.

**Not yet browser-verified this stage:** open Bloody Mary (Practical
Version) and Zombie (Home Bar Spiced & Dark Spec), confirm each now shows
the base's card alone with the note in a separate "How this version
differs" block below it (not a caption on the base card); open Bloody
Mary and Zombie, confirm "Variations" still shows each variation's own
note as a caption under its own card, unchanged from V.3; confirm the
...
## Last completed chunk (Linked Variations Stage V.4 implemented - makeability-aware presentation + first real catalogue links, 2026-09-13 — domain/UI changes + editor wording polish + 2 real recipe_relationships rows via save_recipe(), no new migration)

**Preceded by manual verification of V.2 and V.3.** The user confirmed:
"Variation of" assignment saves correctly; the "How it differs" note
works; variation detail correctly shows the base recipe; base detail
correctly shows its variation; navigation works; the reused full
`CocktailCard` in the relationship section looks good and should be
preserved; the related card correctly shows that recipe's own live
availability/makeability status. V.4 was scoped to preserve this exactly,
not redesign it.

**1. Makeability-aware ordering.** `resolveRecipeVariationContext()`
(`src/domain/recipeRelationships.js`) now sorts `variations` by the
shared `DISPLAY_TIER_ORDER` (`src/domain/availabilityGroups.js`) first -
perfect > good > adapted > almost > unavail, the exact order every other
"grouped by makeability" view in this app already uses - with V.3's
alphabetical order kept as the tie-break inside a tier. No new ranking
system; a small `tierRank()` helper just looks a tier up in the existing
shared array.

**2. "Can't make the original?" framing.** New exported
`isPossibleTier(tier)` in `src/domain/makeabilityCounts.js` - extracted
from `summarizeMakeability()`'s own existing inline rule ("perfect/good/
adapted count as possible, almost/unavail don't") so this is read from
one place, not re-derived. New `shouldShowMakeableVariationFraming(recipe,
variations)` in `src/domain/recipeRelationships.js`: true only when the
CURRENT recipe's own tier is not possible and at least one direct
variation's tier IS - false when the base is already possible (strict OR
adapted; "the difference is merely strict vs adapted but both already
possible" is explicitly not a reason to show the line, per the request),
false when no variation is possible either, false for a missing recipe.
Belongs on the base → variations side only - a variation's own "Variation
of" block never gets an equivalent line about its base. `DetailScreen.jsx`
computes the boolean once (reusing `c`'s own already-computed
`display.tier` and each variation's own already-computed tier - no new
availability computation) and passes it to `VariationsSection.jsx` as a
plain `showMakeableFraming` prop, rendered as one compact `text-cyan`
line inside "Variations" only, above the grid.

**3. Editor wording polish (small, isolated, included per the request's
own explicit allowance).** `EditorScreen.jsx`'s "Variation of" label now
reads **"Based on / variation of (optional)"** with helper text making
the direction explicit ("If this recipe is a variation of another
cocktail, select the original/base recipe..."). Copy-only change - no
field, behavior, or payload shape changed. **"How it differs (optional)"**
left exactly as it was.

**Not built - recorded as a separate follow-up in the plan doc's new
"Manual-test findings" section:** a sticky recipe-name header in the
editor while scrolling deep into Edit Recipe. Out of scope for V.4's own
explicit instructions (no editor restructuring).

**4. Real catalogue relationships - both written through the app's own
normal path, never a raw `insert` bypassing RLS.** Used
`npx supabase db query --linked` to call the existing `save_recipe()` RPC
directly (the same RPC `EditorScreen.jsx`'s Save button calls) under a
simulated real authenticated admin identity - `set local role
authenticated; set local request.jwt.claims = '{"sub":"<admin id>",
"role":"authenticated"}'`, the same identity-switching technique
`supabase/tests/rls_suite.sql` already uses - so RLS's actual
`recipe_is_editable` rule (admin-only for a classic recipe) is what
authorized each write, not superuser SQL access:
- **Bloody Mary (Practical Version) → Bloody Mary** - already existed
  (the user created this through the real editor during V.2/V.3 manual
  verification). Inspected, not duplicated: note reads "Uses Soy sauce
  instead of Worcestershire Sauce and regular salt instead of Celery
  Salt. More common at home" - accurate, left unchanged.
- **Zombie (Home Bar Spiced & Dark Spec) → Zombie** - newly linked.
  Compared both recipes' actual `recipe_components`/steps/description via
  direct SQL before writing anything: the variation uses 2 rums (Dark +
  Spiced, 45 ml each) instead of 3 (Dark + Gold + Demerara); Simple Syrup
  (15 ml) instead of Falernum (15 ml) + Cinnamon Syrup (5 ml); omits
  Pernod (0.3 ml) entirely; uses "Aromatic Bitters" vs "Angostura
  Bitters" (a different specific ingredient_type row, same role); is
  shaken over solid ice and strained over crushed ice per the steps,
  vs. the base's blender + pulse-blend method (no blender step at all in
  the variation). Both are classic/shared/active, same family, exactly
  matching the V.1 catalogue audit's own candidate-pair note. Genuine,
  meaningful differences - not a cosmetic rename - so this is linked, with
  note: "Uses Dark Rum and Spiced Rum instead of Dark, Gold, and Demerara
  Rum, and Simple Syrup instead of Falernum, Cinnamon Syrup, and Pernod.
  Shaken and strained over ice instead of blended - easier with common
  home-bar ingredients and no blender needed." Written by rebuilding the
  variation's own current `p_fields`/`p_components`/`p_taste_tag_ids` via
  SQL directly from its existing rows (a like-for-like re-save, not
  hand-typed) plus the new `p_variation_of` - verified by a follow-up
  query that the recipe's own components/amounts/roles are byte-for-byte
  identical to what they were immediately before the call.

**Verified:** no new migration this stage (both writes went through the
existing, already-RLS-suite-covered `save_recipe()` RPC - re-running the
RLS suite or `db advisors` wasn't needed, and wasn't run). `corepack
pnpm@10.34.3 test` **375/375** (+13: 2 tier-ordering tests +
8 `shouldShowMakeableVariationFraming()` tests in
`recipeRelationships.test.js`; 3 `isPossibleTier()` tests in
`makeabilityCounts.test.js`). `corepack pnpm@10.34.3 build` clean (181
modules, unchanged module count - no new file this stage). Diff reviewed
for V.4 scope creep before committing - none found: no recursive family
tree, no siblings/grandparents, no recipe inheritance, no import support,
no Library/Home variation badge, no availability propagation, no special
variation availability tier, no new relationship type; `computeAvail()`/
`computeMakeability()`/adaptation tiers/Buy Next/Home/Library
grouping/recipe counts all confirmed untouched by inspection.

**Not yet browser-verified this stage:** open Bloody Mary (Practical
Version) and Zombie (Home Bar Spiced & Dark Spec) and confirm each shows
"Variation of" with the correct base + note, and each base shows
"Variations"; toggle My Bar ownership so one side of a pair is possible
and the other isn't, confirm the "Can't make the original?" line appears
...
## Last completed chunk (Linked Variations Stage V.3 implemented - Cocktail Detail relationship UI + navigation, 2026-09-13 — 1 new component + 2 app files + domain resolver ordering fix, no migrations/catalogue changes)

**What shipped.** Member-facing UI for Linked Variations: a recipe that is a
variation shows **Variation of** [base recipe]; a recipe with direct
variations shows **Variations** [variation recipe(s)]; a recipe in the
middle of a chain shows both at once. One-hop only, exactly matching
`resolveRecipeVariationContext()`'s own rule - never a grandparent/sibling/
recursive family tree.

**New file:** `src/components/detail/VariationsSection.jsx` - renders `null`
(no section, no empty heading) when there's neither a base nor variations;
otherwise reuses the existing `CocktailCard` in the exact same grid
`IngredientDetailScreen.jsx`'s own "cocktails using this" section already
established (`grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5`, a small
muted caption under each card for its optional `note`, shown verbatim -
never a generated "uses X instead" description, since the relationship
carries no ingredient-delta data to generate one from). Calls
`useNavigate()` internally (matching `ActionButtons.jsx`'s own precedent)
and navigates each card via `navigate(\`/library/${id}\`)`, the real
`/library/:id` route - only the card itself is clickable, not the whole
section, and each target recipe lands on its own independent detail page
with its own real availability badge (a makeable variation never makes its
base read as makeable, and vice versa - confirmed by inspection that
`computeAvail()`/`computeMakeability()`/`recommendations.js` have no
cross-recipe parameter this could even attach to).

**`src/screens/DetailScreen.jsx`:** added `recipeRelationships` to the
existing `useOutletContext()` destructure (already fetched once alongside
recipes since V.2 - **zero new data loading**, no new Supabase call of any
kind this stage); builds a local `recipesById` map from the already-loaded
`computed` array via a plain `useMemo` (no request, no per-card/N+1 fetch);
calls V.1's own `resolveRecipeVariationContext(c.id, recipeRelationships,
recipesById)` and renders `<VariationsSection base={variationBase}
variations={variations} />` right after `StepsSection`, before
`ActionButtons` - supplementary position in the information hierarchy, not
a hero feature.

**`src/domain/recipeRelationships.js`:** `resolveRecipeVariationContext()`'s
`variations` array now sorted by `recipe.name.localeCompare()` before
returning - the flat `recipe_relationships` fetch has no `ORDER BY`, and
Postgres never promises row order without one, so this was needed to keep
"Variations" rendering in the same order across reloads. Plain alphabetical
determinism only, deliberately NOT makeability-aware ranking (assigned to
V.4, not V.3, per the user's own explicit scope boundary).

**New tests** in `src/domain/recipeRelationships.test.js`: a chain-middle
recipe (A←B←C, resolving B) returns both its base (A) and its variations
([C]) at once; a base with multiple direct variations returns all of them
sorted deterministically by name regardless of the input rows' own order.

**Verified:** `corepack pnpm@10.34.3 test` **362/362** (+2 from this
stage). `corepack pnpm@10.34.3 build` clean (181 modules). Diff reviewed
for V.3 scope creep - none found; no migration, no RLS change, no touch to
`computeAvail()`/`computeMakeability()`/adaptation tiers/Buy Next/Home/
Library grouping/recipe counts. **Not yet browser-verified** - manual check
owed using a test/fixture relationship (not a live catalogue link): a
variation's page shows "Variation of" + note, its base's page shows
"Variations", a chain's middle recipe shows both, tapping either navigates
correctly with normal back-navigation, and a recipe with no relationship
shows neither section.

**Not built (explicitly out of scope for V.3):** no live Bloody Mary/Zombie
...
## Last completed chunk (Linked Variations Stage V.2 atomicity fix - one save_recipe() RPC replaces set_recipe_variation_of(), 2026-09-13 — 1 new migration + service-layer rewrite + RLS suite, no catalogue changes)

**Trigger:** before approving V.2, the user identified that the first
pass's atomicity fix was incomplete. `set_recipe_variation_of()`
correctly made the relationship's own delete-then-insert atomic, and
calling it first in `updateRecipe()` correctly stopped a cyclic
*rejection* from letting later recipe writes proceed - but it did **not**
make the whole editor Save atomic: if the relationship write *succeeded*
and a *later* step (the recipes UPDATE, or a component/tag write) then
failed, the relationship had already committed, half-updated relative to
the rest of the recipe - exactly the state this feature exists to
prevent. The user explicitly ruled out fixing this with a client-side
compensating write and asked for the smallest real database/RPC fix,
extending an existing atomic-save RPC if one existed or introducing the
narrowest new one.

**Root architectural issue:** `createRecipe()`/`updateRecipe()`
(`src/services/recipes.js`) have never been backed by a real database
transaction - each is a sequence of independent Supabase calls (the
recipe's own field update, a component delete+reinsert, a taste-tag
delete+reinsert), a limitation this project has known about and accepted
since before this feature (`updateRecipe()`'s own long-standing comment:
"No client-side multi-statement transaction is available"). The first
V.2 pass added real atomicity to ONE of those steps (the relationship)
without addressing the others - correct in isolation, insufficient as a
fix for "the whole Save is one operation."

**Final transactional design:** `set_recipe_variation_of()` is dropped
(migration `20260913140000`); a new `save_recipe(p_recipe_id, p_fields,
p_components, p_taste_tag_ids, p_variation_of)` function now owns
**everything** the editor's Save changes - the recipe's own fields, its
full component (+ alternative) set, its full taste-tag set, and its one
relationship row - inside a single plpgsql function body, mirroring
`save_ingredient_type()`'s own already-established pattern exactly
(`SECURITY INVOKER` - RLS on every touched table is still the real gate,
unchanged; `search_path=''` with every reference fully qualified;
delete-then-insert-if-present per child table; an explicit `v_touched =
0` check on the recipes UPDATE raising `insufficient_privilege`, same
convention as `save_ingredient_type()`'s own non-owner check). A failure
anywhere - a bad ingredient reference, the caller not owning the recipe,
or the relationship's own V.1 cycle-prevention trigger - rolls back
**everything** the function already did in that call, regardless of
which statement failed or in what order they ran.
...
guarantee now also covers recipe creation - "apply the same principle to
creation where practical," done by extending the one function rather
than redesigning the create path. `createClassicRecipes()` (batch
import) is **unchanged** - a separate, already-per-row-isolated flow
with no relationship support at all, out of scope for this fix (per the
plan's own Import decision).

`src/services/recipes.js`: `createRecipe()`/`updateRecipe()` now call
`save_recipe()` instead of their old per-table sequences;
`insertRecipeWithRelations()`/`insertComponentsWithAlternatives()` are
untouched but now used ONLY by `createClassicRecipes()`. No changes
needed to `EditorScreen.jsx` - the JS-facing function signatures
(`createRecipe(payload)`/`updateRecipe(id, payload)`, including
`variationOf`) are unchanged, only their internals.

**Regression test proving the actual rollback (RLS suite):** fixture
recipes H/I/J with I already a variation of H (note: "original note") and
J a variation of I (a valid chain H←I←J). Calling `save_recipe(I, {name:
"SHOULD NOT PERSIST", ...}, ..., {base_recipe_id: J, ...})` - reassigning
I's base to J would close I→J→I into a cycle (J's own base is I) - is
rejected by the trigger. **Asserted afterward: I's name is still its
original value, and I's relationship is still base=H, note="original
note"** - neither the name change nor the relationship change persisted,
proving the whole function rolled back together, not just the
relationship's own delete-then-insert. Also covers: a successful save
changes the recipe's name AND assigns a new (non-cyclic) base together;
relationship removal (`p_variation_of` null) is part of a successful
save; the create path returns a new recipe owned by the caller; a
non-editor is denied and changes nothing; anon has no EXECUTE grant.

**Verified:** migration `20260913140000_save_recipe.sql` pushed clean via
`supabase db push --linked`. `supabase db advisors --type security` - no
new finding (not `SECURITY DEFINER`, so the linter's generic warning
class doesn't apply; explicitly revoked/re-granted anyway for
consistency). `corepack pnpm@10.34.3 test` **360/360** (unchanged - no
domain logic changed, only the service layer and SQL). `corepack
pnpm@10.34.3 build` clean (180 modules, unchanged). `supabase/tests/
rls_suite.sql`'s `set_recipe_variation_of()` block replaced with the
`save_recipe()` block above - **full suite passes**. Diff reviewed for
scope creep before committing - exactly 1 new migration + the service
layer + the RLS suite changed (no `EditorScreen.jsx`/UI change needed);
...
**Note (2026-09-13): this chunk's own atomicity design (`set_recipe_variation_of()`,
called first in `updateRecipe()`) was found incomplete before the user
approved V.2 and was replaced the same day - see the chunk entry directly
above ("Linked Variations Stage V.2 atomicity fix") for what actually
ships now. Kept below unedited as historical record of the first pass
and exactly why it wasn't sufficient.**
...
## Last completed chunk (Linked Variations Stage V.1 implemented - schema + cycle-prevention trigger + pure domain resolver, 2026-09-13 — 1 migration + RLS suite + 2 new domain files, no catalogue changes)

**Scope: exactly V.1** - the storage + resolution foundation only.
Explicitly not this turn: V.2 (recipe editor field), V.3 (Cocktail Detail
UI), V.4 (makeability-aware presentation + the real Bloody Mary/Zombie
catalogue links), import support. No live Bloody Mary/Zombie relationship
was created.

**Cycle-rule correction, applied before/during implementation:** the
approved plan originally argued a relationship cycle is "inert" under the
one-hop display model and therefore not worth preventing in the database.
The user corrected this before implementation began: Linked Variations
are semantic parent/base facts, so a cycle is invalid catalogue data
regardless of what the member UI happens to resolve - self-links must be
rejected, assigning a base must be rejected if it would create a cycle of
any length, chains may otherwise exist, member-facing resolution stays
one hop only. `docs/plans/linked-variations.md` records this explicitly
(the original reasoning kept struck through as historical record,
immediately followed by the corrected rule and what V.1 actually built).

**Migration `20260913120000_recipe_relationships.sql`:** the
`recipe_relationships` table (`recipe_id` = the variation, `related_
recipe_id` = its base, optional `note` ≤280 chars, both FKs `on delete
cascade`, `unique(recipe_id)` so a variation has at most one base, `check
(recipe_id <> related_recipe_id)`); an index on `related_recipe_id` for
the reverse "variations of this base" lookup (`recipe_id` already has one
via the unique constraint); RLS reusing the existing `recipe_is_visible()`/
`recipe_is_editable()` SECURITY DEFINER helpers as-is - read requires both
sides visible to the viewer, insert/delete gated on editing the
*variation* (re-affirms the approved decision that a base recipe's owner
does not control other members' variations), no update policy (change =
delete + re-insert, matching `recipe_component_alternatives`'s own
precedent).

**Cycle prevention:** a new `BEFORE INSERT OR UPDATE` trigger,
`forbid_recipe_relationship_cycle()` (`SECURITY DEFINER`, revoked from
public/anon/authenticated - same discipline as every other SECURITY
DEFINER function in this codebase, e.g. `sync_recipe_favorite_count()`).
Cheap to implement correctly here specifically because `unique(recipe_id)`
already guarantees the whole relationship graph is a forest of trees (at
most one outgoing edge per node) - checking whether a proposed new edge
would close a loop is a single linked-list walk upward from the proposed
base, not a general graph traversal or a recursive CTE; a generous fixed
depth cap (10000) is a defensive guard against corrupted data, not a real
expectation of ever being hit. A self-link is caught by the same trigger
as a degenerate zero-length cycle (the walk's very first check); the
original `check` constraint stays in place too, as a structural backstop
independent of trigger state.

**New `src/domain/recipeRelationships.js`** (pure,
`resolveBaseRelationship()`/`resolveDirectVariations()`/
`resolveRecipeVariationContext()`) - one-hop resolution only, never
recursively walks a chain; the screen-ready function additionally looks
each side up in a caller-supplied `recipesById` map and drops (never
throws on) a missing/stale/invisible target. Not yet wired to any
fetch/service or UI - that's V.2/V.3's job; this stage only proves the
storage + resolution foundation works. **16 new tests** covering: base
lookup and direct-variations lookup both resolve correctly; a chain
(A←B←C) never gets flattened past one hop in either direction; multiple
variations on one base; null-safety for missing/empty input; a
defensively-first-match behavior if fed malformed multi-base input; a
missing recipe in the lookup is dropped, not thrown on; unlinking (an
empty relationships array) never touches the recipe objects themselves
(same object references, asserted directly).

**RLS suite (`supabase/tests/rls_suite.sql`) new block:** the variation's
own editor can link it to a visible base (note round-trips); a self-link
is rejected (caught as a degenerate cycle - required updating the test's
expected exception class from `check_violation` to `raise_exception`
once live testing showed the BEFORE trigger fires before the CHECK
constraint is ever evaluated); a second base for an already-linked
variation is rejected (unique violation); a valid non-cyclic chain
(A←B←C) is allowed; a direct cycle (A→B→A) is rejected; a longer 3-hop
cycle (A→C→B→A) is rejected; one base can have multiple direct
variations; a non-editor cannot link someone else's recipe; the link is
invisible to a viewer who can't see both sides; anon is denied on
read/insert (using a deliberately non-cycle-forming pair for that
specific assertion, found necessary after the first attempt accidentally
reused a cycle-forming pair and got masked by the trigger instead of
testing RLS); unlinking leaves both recipes' own rows completely
untouched; deleting the base recipe cascades and removes the relationship.
**Full suite passes.**

**Verified:** migration pushed clean via `supabase db push --linked`.
`supabase db advisors --type security` - no new finding (the new
function is correctly revoked from all three roles, so the linter's
generic "SECURITY DEFINER function is authenticated/anon-executable"
warning class - which every pre-existing app RPC in this project already
triggers and has always accepted - does not fire for this one).
`corepack pnpm@10.34.3 test` **357/357** (+16 new). `corepack
pnpm@10.34.3 build` clean (178 modules - unchanged from before this
chunk, since the new domain files aren't imported by any app code yet).
Diff reviewed for scope creep before committing - exactly 1 migration +
...
## Last completed chunk (Ingredient Detail page — planning only, 2026-09-12 — `docs/plans/ingredient-detail-page.md` created, no code/migrations/catalogue changes)

**Trigger:** the user identified normal recipe-browsing friction - finding
a missing ingredient on a recipe page (Bloody Mary / Tomato Juice example)
means leaving the recipe entirely to fix My Bar in a separate flow, then
navigating back. Asked for ingredients to become navigable first-class
objects, with a compact per-ingredient page (identity, My Bar action,
relationships, homemade preparation, cocktails using it, admin link) -
planning only this turn.

**Audit finding that reshaped the whole plan:** an ingredient/bottle
detail screen **already exists and is already routed and linked** -
`IngredientDetailScreen.jsx` (`/bar/type/:id`, `/bar/product/:id`, shipped
as item 16's Stage 3/4), already reached by tapping a type tile or product
name in My Bar (`MyBarScreen.jsx` already calls
`navigate(\`/bar/type/${type.id}\`)`; `TypeCard.jsx` already keeps
card-body-tap (navigate) and the dedicated 44px ownership checkmark as two
deliberately separate handlers - "exploring an ingredient must never risk
silently changing what's owned"). **This feature enriches that existing
screen rather than building a new one** - the plan doc documents this
explicitly so it isn't rebuilt from scratch.

**Also found while auditing `docs/my-bar-ux-plan.md`/this file's own
history:** an earlier My Bar redesign stage *did* put an inline admin
`IngredientTypeEditor` directly on this same screen, and a *later* stage
deliberately replaced it with a single admin-only "Edit ingredients"
shortcut to Admin → Ingredient Types in general (not deep-linked to the
specific type). This plan's own admin affordance (Stage I.4) follows that
same lesson - a plain link out, not a reintroduced inline editor or a new
deep-link mechanism.

**Reuse confirmed for every section, zero new domain logic:**
`computeMakeability()`/`display` (Stage D.1, already on every `computed`
recipe), `findRecipesUsingIngredient()` (unchanged, still viewing-blind by
design), `groupByDisplayTier()`/`DISPLAY_TIER_ORDER` (Stage D.2 - the
ingredient-detail page's own grouping currently still uses raw `avail`,
predating Stage D; switching it to the shared function is part of Stage
I.1), the single shared `useInventory()` instance (`toggleType`/
`toggleProduct` - already what every other My Bar surface calls, so
"ownership changes reach the originating cocktail automatically" is free,
not a new mechanism), and `isPreparationSatisfiable()` (Stage D.3).
`catalog.formConversions`/`ingredientSubstitutions`/`ingredientPreparations`/
`ingredientPreparationInputs`/`categories` are all already fetched once and
shared via `AppShell`'s outlet context - no new query.

**No schema changes needed at all**, including for the optional short
description mentioned in the request - **`ingredient_types.description`
already exists** (added for batch import), just was never rendered to a
member; this feature only needs to display it, not add a column.

**Proposed stages** (resequenced from the user's own suggestion, since the
route/page already exists so there's no "just scaffold it" stage needed):
**I.1** tappable ingredient links from `DetailScreen`'s ingredient list +
switch the existing page's grouping to the shared `display.tier` (the
actual friction fix, first) → **I.2** My Bar Add/Remove action → **I.3**
identity/category/description + Can-provide/Can-be-replaced-by +
preparation display → **I.4** admin-only "Edit ingredient" link. Explicit
deferrals recorded in the plan doc: deep-linking the admin action to a
specific type, linking a substitute/adapted-match's own name inside a row,
the reverse "provided by" relationship direction, a Buy-Next-style
per-ingredient unlock/restore annotation, and everything else the user's
own scope-boundary list already excluded (Linked Variations, preparation
chains, inventory quantities, AI descriptions, automatic substitutions,
unrelated My Bar redesign).
...
## Last completed chunk (Substitutes & Variations — Stage D manual verification PASSED + adapted-category rename, 2026-09-12 — `src/**` only, no migrations, no catalogue changes)
...
adapted cocktails (Gin Fizz) also correctly discovered. Owning Lemon Juice
does **not** imply owning whole Lemon (directional Can-provide, as
designed). Home showed **"10 cocktails possible · 5 ready, 5 with
substitutions or preparation."** The prior screenshot showing the Daiquiri
unavailable was expected behavior at the time (Lemon Juice wasn't owned
yet) - **explicitly not a bug; no investigation or fix was needed or
made.**

**Terminology cleanup: the adapted discovery CATEGORY renamed "Make With
Adaptations"** (was "Make With Substitutions") - the category also holds
cocktails resolvable through a homemade preparation alone with no
substitute involved (e.g. missing only Simple Syrup), which "...
Substitutions" under-described once tier 5 shipped. Centralized in one
new export, `ADAPTED_CATEGORY_LABEL` (`src/data/constants.js`), consumed
by `AVAIL_FILTERS`'s `adapted` entry (shared Library/Lists filter),
Library's `AVAIL_GROUP_LABEL.adapted` (grouped-view heading), and Home's
section heading - one source, same wording everywhere.

**Deliberately NOT renamed** (no genuine technical reason to, and not what
was asked): the `display.tier === "adapted"` identifier and everything
built on it (`computeAdaptedResult`, `rankAdapted`, `AVAIL_CFG.adapted`/
`AVAIL_TONE.adapted`, `computeMakeability()`'s `adapted` field); and a
cocktail's own composed status text ("Make with substitutions" / "Prepare
X first" / both joined - `composeAdaptedLabel()` in
`src/domain/makeability.js`, plus the same string as `AVAIL_CFG.adapted
.label`'s fallback in `primitives.jsx`) - that's a different, more specific
mechanism describing what ONE cocktail needs, not the category name, and
stays exactly as it was.

Comments in `src/domain/almostThere.js`, `src/screens/HomeScreen.jsx`, and
`src/screens/ListsScreen.jsx` that quoted the old category name were
updated to match; two comments that explicitly document the rename itself
(as a historical note) still mention the old name on purpose. This
planning doc's own Stage D.1-D.5 write-ups still say "Make With
Substitutions" throughout, since that was accurate at the time each was
written - left as historical record, not rewritten (see the plan doc's own
"Stage D manual verification — PASSED" section for the full finalization
note).

**Verified:** `corepack pnpm@10.34.3 test` **311/311** (unchanged - no test
asserted the exact old string, nothing needed updating). `pnpm build`
